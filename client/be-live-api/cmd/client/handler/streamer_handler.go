...
func (h *streamHandler) initializeStream(c echo.Context) error {
	...
	if err != nil {
		return utils.BuildErrorResponse(c, http.StatusBadRequest, err, fmt.Sprintf("thumbnail field is required: %s", err.Error()), "INVALID_REQUEST")
	}

	if file.Size > utils.MaxImageSize {
		return utils.BuildErrorResponse(c, http.StatusBadRequest, errors.New("file size exceeds the maximum allowed limit of 1MB"), nil, "INVALID_REQUEST")
	}
	...
	if !isImage {
		return utils.BuildErrorResponse(c, http.StatusBadRequest, errors.New("file is not an image"), nil, "INVALID_REQUEST")
	}
...
func (h *streamHandler) getStream(c echo.Context) error {
	...
	if err != nil {
		return utils.BuildErrorResponse(c, http.StatusBadRequest, errors.New("invalid id parameter"), nil, "INVALID_REQUEST")
	}
	...
	if err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return utils.BuildErrorResponse(c, http.StatusNotFound, err, nil, "NOT_FOUND")
		}
	...
	if stream.Status == model.PENDINGSOFTWARE {
		return utils.BuildErrorResponse(c, http.StatusBadRequest, errors.New("streamer is not live yet"), nil, "INVALID_REQUEST")
	}
...
func (h *streamHandler) addView(c echo.Context) error {
	...
	if err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return utils.BuildErrorResponse(c, http.StatusNotFound, errors.New("stream not found"), nil, "NOT_FOUND")
		}
		return utils.BuildErrorResponse(c, http.StatusInternalServerError, err, nil, "INTERNAL_ERROR")
	}

	if stream.Status != model.ENDED {
		return utils.BuildErrorResponse(c, http.StatusBadRequest, errors.New("this isn't a video, you can't add views"), nil, "INVALID_REQUEST")
	}
...
func (h *streamHandler) actionLike(c echo.Context) error {
	...
	if stream.Status != model.ENDED {
		return utils.BuildErrorResponse(c, http.StatusBadRequest, errors.New("this isn't a video, you can't react"), nil, "INVALID_REQUEST")
	}
...
func (h *streamHandler) getComments(c echo.Context) error {
	...
	if c.QueryParam("page") != "" {
		...
		if err != nil {
			return utils.BuildErrorResponse(c, http.StatusBadRequest, errors.New("invalid page parameter"), nil, "INVALID_REQUEST")
		}
	}
	...
	if c.QueryParam("limit") != "" {
		...
		if err != nil {
			return utils.BuildErrorResponse(c, http.StatusBadRequest, errors.New("invalid limit parameter"), nil, "INVALID_REQUEST")
		}
	}
...
func (h *streamHandler) updateComment(c echo.Context) error {
	...
	if err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return utils.BuildErrorResponse(c, http.StatusNotFound, errors.New("comment not found"), nil, "NOT_FOUND")
		}
		return utils.BuildErrorResponse(c, http.StatusInternalServerError, err, nil, "INTERNAL_ERROR")
	}

	if claims.ID != comment.UserID {
		return utils.BuildErrorResponse(c, http.StatusUnauthorized, errors.New("not authorized"), nil, "UNAUTHORIZED")
	}
...
func (h *streamHandler) bookmark(c echo.Context) error {
	...
	if stream.Status != model.ENDED {
		return utils.BuildErrorResponse(c, http.StatusBadRequest, errors.New("this isn't a video, you can't save"), nil, "INVALID_REQUEST")
	}
...
func (h *streamHandler) deleteBookmark(c echo.Context) error {
	...
	if stream.Status != model.ENDED {
		return utils.BuildErrorResponse(c, http.StatusBadRequest, errors.New("this isn't a video, you can't delete"), nil, "INVALID_REQUEST")
	}
...
func (h *streamHandler) addShare(c echo.Context) error {
	...
	if stream.Status != model.ENDED {
		return utils.BuildErrorResponse(c, http.StatusBadRequest, errors.New("this isn't a video, you can't add shares"), nil, "INVALID_REQUEST")
	}
...
func (h *streamHandler) getChannel(c echo.Context) error {
	...
	if user.Role.Type != model.STREAMER {
		return utils.BuildErrorResponse(c, http.StatusInternalServerError, errors.New("this isn't a streamer"), nil, "INTERNAL_ERROR")
	}
