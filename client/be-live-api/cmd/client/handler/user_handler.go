...
func (h *userHandler) GetInformation(c echo.Context) error {
	...
	if err != nil {
		return utils.BuildErrorResponse(c, http.StatusInternalServerError, errors.New("can't get user information"), nil, "INTERNAL_ERROR")
	}
...
func (h *userHandler) UpdateUser(c echo.Context) error {
	...
	if req.Password != "" && req.NewPassword != "" {
		if req.Password == req.NewPassword {
			return utils.BuildErrorResponse(c, http.StatusInternalServerError, errors.New("current password and new password are the same"), nil, "INTERNAL_ERROR")
		}

		//Check password
		if !utils.CheckPasswordHash(req.Password, user.PasswordHash) {
			return utils.BuildErrorResponse(c, http.StatusInternalServerError, errors.New("current password is incorrect"), nil, "INTERNAL_ERROR")
		}
...
func (h *userHandler) ChangeTwoFAStatus(c echo.Context) error {
	...
	if pTwoFa == nil {
		if !*req.IsEnabled {
			return utils.BuildErrorResponse(c, http.StatusInternalServerError, errors.New("you need to enable 2fa first"), nil, "INTERNAL_ERROR")
		}
		...
		if !pTwoFa.Is2faEnabled && pTwoFa.Secret != "" {
			return utils.BuildErrorResponse(c, http.StatusInternalServerError, errors.New("you need to verify with otp first"), nil, "INTERNAL_ERROR")
		} else { // after user verify
			// user want to enable and already enabled in db
			if *req.IsEnabled && pTwoFa.Is2faEnabled {
				return utils.BuildErrorResponse(c, http.StatusBadRequest, fmt.Errorf("2fa is already enabled"), nil, "INVALID_REQUEST")
			} else if !*req.IsEnabled && !pTwoFa.Is2faEnabled { // user want to disable and already disabled in db
				return utils.BuildErrorResponse(c, http.StatusBadRequest, fmt.Errorf("2fa is already disabled"), nil, "INVALID_REQUEST")
...
func (h *userHandler) VerifyTwoFA(c echo.Context) error {
	...
	twoFA, err := h.srv.TwoFA.GetTwoFAInfo(claims.ID)
	if err != nil {
		return utils.BuildErrorResponse(c, http.StatusInternalServerError, errors.New("can't get the secret key"), nil, "INTERNAL_ERROR")
	}
