...
func (h *wsHandler) StreamLive(c echo.Context) error {
	...
	if err != nil {
		log.Println(err)
		return utils.BuildErrorResponse(c, http.StatusUnauthorized, err, nil, "UNAUTHORIZED")
	}
	if claims.RoleType != model.STREAMER {
		log.Println(err)
		return utils.BuildErrorResponse(c, http.StatusUnauthorized, errors.New("you haven't authorized the role streamer"), nil, "UNAUTHORIZED")
	}
...
func (h *wsHandler) handleUserInteraction(c echo.Context) error {
	...
	if err != nil || stream == nil {
		return utils.BuildErrorResponse(c, http.StatusBadRequest, fmt.Errorf("stream with id %d not found", id), nil, "INVALID_REQUEST")
	}

	if stream.UserID != claims.ID && stream.Status != model.STARTED {
		return utils.BuildErrorResponse(c, http.StatusBadRequest, fmt.Errorf("stream with id %d is not live", id), nil, "INVALID_REQUEST")
	}
