...
func SkipMiddleware(next echo.HandlerFunc) echo.HandlerFunc {
	return func(c echo.Context) error {
		...
		if authHeader == "" {
			return BuildErrorResponse(c, http.StatusUnauthorized, errors.New("missing token"), nil, "UNAUTHORIZED")
		}

		// Validate token
		tokenParts := strings.Split(authHeader, " ")
		if len(tokenParts) == 2 && tokenParts[0] == "Bearer" && tokenParts[1] == "" {
			return BuildErrorResponse(c, http.StatusUnauthorized, errors.New("invalid token format"), nil, "UNAUTHORIZED")
		}

		tokenString := strings.Trim(strings.ReplaceAll(authHeader, "Bearer", ""), "* ")
		claims, err := ValidateAccessToken(tokenString)
		if err != nil {
			return BuildErrorResponse(c, http.StatusUnauthorized, errors.New("invalid or expired token"), nil, "UNAUTHORIZED")
		}

		if claims.Status == model.BLOCKED {
			return BuildErrorResponse(c, http.StatusUnauthorized, errors.New("account is locked and can't be used"), nil, "UNAUTHORIZED")
		}
	...
	return func(next echo.HandlerFunc) echo.HandlerFunc {
		return func(c echo.Context) error {
			...
			if model.STREAMER != roleType {
				return BuildErrorResponse(c, http.StatusUnauthorized, errors.New("you haven't authorized the role streamer"), nil, "UNAUTHORIZED")
			}
	...
	return func(next echo.HandlerFunc) echo.HandlerFunc {
		return func(c echo.Context) error {
			...
			if !slices.Contains([]model.RoleType{model.ADMINROLE, model.SUPPERADMINROLE}, roleType) { // super admin and admin
				return BuildErrorResponse(c, http.StatusUnauthorized, errors.New("you haven't authorized the role admin"), nil, "UNAUTHORIZED")
			}
			// Call the next handler
			return next(c)
		}
