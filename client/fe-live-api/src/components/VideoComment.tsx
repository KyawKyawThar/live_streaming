...
const VideoComment = ({ videoId }: ComponentProps) => {
  ...
  const handleDeleteComment = (commentId: number) => {
    openConfirmModal(
      t('watchVideo.ConfirmToDeleteCommentTitle'),
      t('watchVideo.ConfirmToDeleteCommentDesc'),
  ...
  return (
    <div className="space-y-4 mt-5 w-full lg:w-2/3">
      ...
      <h2 className="text-sm font-semibold">
        {totalItems} {t('Comment', { count: totalItems })}
      </h2>

      {/* New Comment Input */}
      <div className="space-y-2">
        <Textarea
          placeholder={`${t('watchVideo.AddComment')}...`}
          value={newComment}
          ...
          <div key={comment.id} className="flex items-start w-full">
            <div className="flex space-x-4 w-full">
              <AppAvatar
                ...
                fallback={comment?.display_name}
              />
              <div className="space-y-1 w-full">
                <div className="flex items-center justify-start space-x-2">
                  <p className="font-semibold text-sm">
                    {comment?.display_name || t('Unknown')}
                  </p>
                ...
                {editingCommentId === comment.id ? (
                  <Fragment>
                    ...
                    <div className="flex justify-end gap-2 mt-2">
                      ...
                      >
                        {isApiCommentUpdating ? `${t('Saving')}...` : t('Save')}
                      </Button>
                    </div>
                  </Fragment>
                ) : (
      ...
      {comments.length === totalItems && (
        <div className="mt-4 flex justify-center text-muted-foreground italic text-sm">
          <p className="p-0">— {t('EndOfComments')} —</p>
        </div>
      )}
