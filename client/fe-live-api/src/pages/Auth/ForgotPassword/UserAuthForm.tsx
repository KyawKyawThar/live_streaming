    ...
    <div className="grid gap-6">
      <Dialog open={isSuccessModalOpen} onOpenChange={handleModalClose}>
        <DialogContent className="sm:max-w-[425px]">
          <DialogHeader>
            <DialogTitle>{t('forgotPassword.ResetSuccess')}</DialogTitle>
          </DialogHeader>
          <p className="mt-2">{t('forgotPassword.ResetSuccessDescription')}</p>
          <DialogFooter>
      ...
      <form onSubmit={handleSubmit(onSubmit)}>
        <div className="grid gap-2 space-y-3">
          <div className="grid gap-1">
            <Label htmlFor="email">
              {t('forgotPassword.Username')} <RequiredInput />
            </Label>
            <Input
              id="email"
          ...
          <div className="grid gap-1">
            <Label htmlFor="email">
              {t('forgotPassword.OTP')} <RequiredInput />
            </Label>
            <Controller
              name="otpCode"
              control={control}
          ...
          <div className="grid gap-1 mt-2">
            <div className="flex justify-between items-center">
              <Label htmlFor="password">
                {t('forgotPassword.Password')} <RequiredInput />
              </Label>
            </div>
            <Input
              id="password"
          ...
          <div className="grid gap-1 mt-2">
            <div className="flex justify-between items-center">
              <Label htmlFor="confirm-password">
                {t('forgotPassword.ConfirmPassword')} <RequiredInput />
              </Label>
            </div>
            <Input
              id="password"
            ...
            {errors?.root?.message && (
              <AppAlert
                className="mt-2"
                variant="destructive"
                title={t('forgotPassword.RootErrorTitle')}
          ...
          <Button type="submit" disabled={isLoading} className="mt-3">
            ...
            {isLoading
              ? t('Processing')
              : t('forgotPassword.ButtonForgotPassword')}
          </Button>
        </div>
      </form>
    </div>
