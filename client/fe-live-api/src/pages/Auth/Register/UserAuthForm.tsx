...
const UserAuthForm = () => {
  ...
  if (emailFailure)
    invalidEmailError = <div>{t('register.EmailRequired')}</div>;
  if (usernameFailure)
    invalidUsernameError = <div>{t('register.UsernameRequired')}</div>;
  if (passwordFailure)
    invalidPasswordError = (
      <div>{t('register.PasswordError', { count: PasswordRules.min })}</div>
    );
  if (confirmPasswordFailure)
    invalidConfirmPasswordError = (
      <div>{t('register.ConfirmPasswordError')}</div>
    );
  if (displayNameFailure)
    invalidDisplayNameError = <div>{t('register.DisplayNameError')}</div>;
  if (accountExisted)
    accountExistedError = <div>{t('register.AccountExistedError')}</div>;
  if (registerFailure) {
      ...
      <form onSubmit={onUserRegister}>
        <div className="grid gap-2">
          <div className="grid gap-1">
            <Label htmlFor="email">
              {t('register.Email')} <RequiredInput />
            </Label>
            <Input
              ref={emailInput}
          ...
          <div className="grid gap-1 mt-2">
            <Label htmlFor="username">
              {t('register.Username')} <RequiredInput />
            </Label>
            <Input
              ref={usnInput}
          ...
          <div className="grid gap-1 mt-2">
            <Label htmlFor="displayName">
              {t('register.DisplayName')} <RequiredInput />
            </Label>
            <Input
          ...
          <div className="grid gap-1 mt-2">
            <Label htmlFor="password">
              {t('register.Password')} <RequiredInput />
            </Label>
            <Input
              ref={pwdInput}
          ...
          <div className="grid gap-1 mt-2">
            <Label htmlFor="confirm-password">
              {t('register.ConfirmPassword')} <RequiredInput />
            </Label>
            <Input
              ref={confirmPwdInput}
          ...
          {somethingWrongError && (
            <AppAlert
              className="mt-2"
              variant="destructive"
              title={t('register.RootErrorTitle')}
              description={somethingWrongError}
            />
          )}
          <Button disabled={isLoading} className="mt-3">
            ...
            {isLoading
              ? t('register.ButtonCreatingAccount')
