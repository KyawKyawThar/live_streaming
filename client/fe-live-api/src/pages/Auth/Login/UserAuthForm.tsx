...
const UserAuthForm = () => {
  ...
  let invalidEmailError = null,
    ...
    blockedError = null;
  if (usernameFailure)
    invalidEmailError = <div>{t('login.EmailRequired')}</div>;
  if (passwordFailure)
    invalidPasswordError = <div>{t('login.PasswordRequired')}</div>;
  if (blocked) blockedError = <div>{t('login.BlockedError')}</div>;
  if (loginFailure) {
    wrongCredentialError = <div>{t('login.LoginError')}</div>;
    invalidEmailError = '';
      ...
      <form onSubmit={onUserLogin}>
        <div className="grid gap-2">
          <div className="grid gap-1">
            <Label htmlFor="email">
              {t('login.Username')} <RequiredInput />
            </Label>
            <Input
              ref={usnInput}
          ...
          <div className="grid gap-1 mt-2">
            <div className="flex justify-between items-center">
              <Label htmlFor="password">
                {t('login.Password')} <RequiredInput />
              </Label>
