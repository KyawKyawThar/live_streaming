...
const WatchVideo = () => {
  ...
  const handleBookmarkVideo = async () => {
    if (videoDetails && videoDetails?.id) {
      if (!isSaved) {
        ...
        if (!isSuccess) {
          setIsSaved(false);
          toast.error(t('ErrorSaveBookmark'));
        }
      } else if (isSaved) {
        setIsSaved(false);
        ...
        if (!isSuccess) {
          setIsSaved(true);
          toast.error(t('ErrorRemoveBookmark'));
        }
      }
    }
  };
  ...
  const handleShare = async () => {
    if (videoDetails && videoDetails?.id) {
      ...
      if (copiedText) toast.success(t('CopyToClipboard'));
    }
  };
    ...
    return (
      <div>
        <NotFoundCentered
          ...
          title={t('feed.NoVideoFound')}
          description={t('VideoUnavailable')}
          redirectTo={{
            Icon: <SquarePlay className="h-4 w-4" />,
            buttonText: t('WatchVideos'),
            link: FEED_PATH,
          }}
        />
      </div>
      ...
      <div className="flex flex-col pt-0 space-y-6 min-h-screen">
        ...
        <div ref={streamerAvatarRef} className="flex items-center">
          <div className="flex items-center space-x-2 flex-1">
            ...
            >
              <AppAvatar
                ...
                fallback={videoDetails?.display_name}
                classes="w-10 h-10"
              />
            </Link>
            <div>
          ...
          <div className="flex space-x-2 items-center">
            <AppButton
              Icon={Share2}
              label={`${sharedCount > 0 ? sharedCount : ''} ${t('Share', {
                count: sharedCount,
              })}`}
              tooltipOnSmallScreens
              size="sm"
            ...
            <AppButton
              ...
              label={isSaved ? t('Bookmarked') : t('Bookmark')}
              tooltipOnSmallScreens
              size="sm"
              variant="outline"
