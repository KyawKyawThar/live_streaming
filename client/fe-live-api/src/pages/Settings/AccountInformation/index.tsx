...
const AccountInformation = () => {
  ...
  const handleFormSubmit = async (
    ...
    if (!!data && !_errors) {
      ...
      openNotifyModal(
        ...
        t('settings.SuccessUpdateProfileTitle'),
        t('settings.SuccessUpdateProfileDescription')
      );
    } else {
      if (_errors) {
  ...
  if (displayNameFailure)
    invalidDisplayNameError = t('settings.DisplayNameError', {
      count: MAX_DISPLAY_NAME_COUNT,
    });
  if (avatarImageFailure)
    invalidAvatarImageError = t('settings.ProfilePhotoError', {
      count: MAX_IMAGE_SIZE_IN_MB,
    });
  if (actionFailure) {
    somethingWrongError = t('live.CommonError');
          ...
          >
            ...
            <div className="flex flex-col-reverse lg:flex-row lg:items-start lg:justify-start gap-4 lg:gap-8 lg:w-1/2">
              ...
              <div className="flex flex-col flex-1 gap-4">
                ...
                <div className="grid gap-3">
                  <Label htmlFor="displayName">
                    {t('settings.DisplayName')} <RequiredInput />
                  </Label>
                  <Input
                    value={displayName}
              ...
              <div className="grid gap-3 lg:items-start">
                <Label htmlFor="description">
                  {t('settings.ProfilePhoto')} <RequiredInput />
                </Label>
                <span className="text-xs text-muted-foreground -mt-1">
                  {t('settings.ProfilePhotoDescription', {
                    count: MAX_IMAGE_SIZE_IN_MB,
                  })}
                </span>
                <ImageUpload
            ...
            <div className="flex gap-2 mt-4">
              <Button type="submit" disabled={isLoading}>
                {isLoading
                  ? t('settings.ButtonUpdatingProfile')
                  : t('settings.ButtonUpdateProfile')}
              </Button>
            </div>
          </form>
        </div>
