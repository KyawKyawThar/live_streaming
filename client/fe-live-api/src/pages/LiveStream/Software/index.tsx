...
const LiveStreamSoftware = () => {
    ...
    if (data.id) {
      ...
      if (mode === FORM_MODE.CREATE) {
        ...
        openNotifyModal(
          ...
          t('live.StreamStartSoftwareSuccessTitle'),
          t('live.StreamStartSoftwareSuccessDescription')
        );
        ...
        openNotifyModal(
          NotifyModalType.SUCCESS,
          t('live.StreamUpdateSuccessTitle'),
          t('live.StreamUpdateSuccessDesc')
        );
    }
  };
  ...
  const handleEndStream = () => {
    openConfirmModal(
      t('live.StreamConfirmToEndTitle'),
      t('live.StreamConfirmToEndDesc'),
  ...
  const handleEndStreamConfirmed = () => {
    ...
    openNotifyModal(
      NotifyModalType.SUCCESS,
      t('live.StreamEndSuccessTitle'),
      t('live.StreamEndSuccessDesc'),
      () => {
        navigate(
  ...
  useEffect(() => {
    if (
      ...
      openNotifyModal(
        NotifyModalType.SUCCESS,
        isLiveEndEventReceived
          ? t('live.StreamForceEndTitle')
          : t('live.StreamEndFromSoftwareTitle'),
        isLiveEndEventReceived
          ? t('live.StreamForceEndDesc')
          : t('live.StreamEndFromSoftwareDescription'),
        () => {
          navigate(
            WATCH_VIDEO_PATH.replace(':id', streamDetails?.id?.toString() || '')
          );
                  ...
                  <div className="relative z-10 flex flex-col justify-center items-center text-center gap-3 bg-white/80 dark:bg-black/80 p-5 backdrop-blur rounded-md">
                    {IS_PENDING_SOFTWARE ? (
                      <div className="flex gap-2 items-center text-sm">
                        <LoadingIndicator width={15} height={15} />{' '}
                        {t('live.SoftwareWaitingText')}
                      </div>
                    ) : (
                    ...
                    {!IS_PENDING_SOFTWARE && (
                      <Fragment>
                        <span className="text-balance font-medium">
                          {t('live.SoftwareSetupDescription1')}
                        </span>
                        <span className="text-sm text-muted-foreground">
                          {t('live.SoftwareSetupDescription2')}
                        </span>
                      </Fragment>
                    )}
                    <Button
                    ...
                    >
                      {t('live.StreamSetupGuide')} <CircleHelp />
                    </Button>
          ...
          <div className="hidden md:inline-block absolute right-5">
            {isStreamStarted && (
              <Button variant="ghost" size="sm" onClick={toggleChat}>
                <MessageSquare /> {isChatVisible ? t('Hide') : t('Show')}{' '}
                {t('Chat')}
              </Button>
            )}
          </div>
