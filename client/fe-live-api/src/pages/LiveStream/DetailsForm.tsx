...
const DetailsForm = (props: ComponentProps) => {
  ...
  if (titleFailure)
    invalidTitleError = t('live.TitleError', {
      count: StreamDetailsRules.title.max,
    });
  if (descriptionFailure) invalidDescriptionError = t('live.DescriptonError');
  if (categoryFailure) invalidCategoryError = t('live.CategoriesError');
  if (thumbnailImageFailure)
    invalidThumbnailImageError = t('live.ThumbnailImageError', {
      count: MAX_IMAGE_SIZE_IN_MB,
    });
  if (actionFailure) {
    somethingWrongError = t('live.TitleError');
    invalidTitleError = '';
  ...
  switch (_mode) {
    case FORM_MODE.CREATE:
      formTitle = t('live.CreateStream');
      formDescription = t('live.CreateStreamDescription');
      break;
    case FORM_MODE.EDIT:
      formTitle = t('live.EditStream');
      formDescription = t('live.EditStreamDescription');
      break;
    case FORM_MODE.VIEW:
      formTitle = t('live.ViewStream');
      formDescription = t('live.ViewStreamDescription');
      break;
  }
      ...
      <DialogContent className="sm:max-w-[600px] p-0">
        <DialogHeader className="p-5 px-7 pb-0">
          ...
          {!isViewMode && categories?.length === 0 && (
            <AppAlert variant="destructive" title={t('live.NoCategoryError')} />
          )}
        </DialogHeader>
        <Separator />
        <div className="grid gap-1 p-5 px-7 pt-2">
          <div className="grid gap-3">
            {somethingWrongError && (
              <AppAlert
                variant="destructive"
                title="Error"
                description={t('live.CommonError')}
              />
            )}
            ...
            {mode === FORM_MODE.VIEW && data && data.push_url && (
              <>
                <Label>{t('live.StreamSoftwareTitle')}</Label>
                      ...
                      children={
                        <div
                          onClick={() => {
                            if (streamServer) {
                              copy(streamServer);
                              toast.success(t('live.StreamServerCopied'));
                            }
                          }}
                      ...
                      children={
                        <div
                          onClick={() => {
                            if (streamKey) {
                              copy(streamKey);
                              toast.success(t('live.StreamKeyCopied'));
                            }
                          }}
            ...
            <div className="grid gap-3">
              <div className="flex justify-between items-center">
                <Label htmlFor="title">
                  {t('live.Title')} {!isViewMode && <RequiredInput />}
                </Label>
              ...
              <Input
                ...
                placeholder={t('live.TitleDescriptoin')}
            ...
            <div className="grid gap-3 mt-1">
              <div className="flex justify-between items-center">
                <Label htmlFor="description">
                  {t('live.Description')} {!isViewMode && <RequiredInput />}
                </Label>
                <p className="text-xs text-muted-foreground">
              ...
              <Textarea
                ...
                rows={4}
                placeholder={t('live.DescriptionDescription')}
                disabled={isLoading || isViewMode}
              />
            ...
            <div className="grid gap-3 mt-1">
              <Label htmlFor="description">
                {t('live.Categories')} {!isViewMode && <RequiredInput />}
              </Label>
              <div className="max-w-xl">
                {isViewMode ? (
                  ...
                  <MultiSelect
                    ...
                    placeholder={t('live.CategoriesDescription', {
            ...
            <div className="flex flex-col-reverse md:flex-row w-full gap-5 justify-start items-start mt-1">
              ...
              <div className="w-full md:w-1/2 grid gap-3">
                <Label htmlFor="description">
                  {t('live.ThumbnailImage')} {!isViewMode && <RequiredInput />}
                </Label>
                {isViewMode ? (
                  <AuthImage
              ...
              <div className="w-full md:w-1/2 flex flex-col items-start gap-3">
                <Label htmlFor="title">
                  {t('live.StreamType')} {!isViewMode && <RequiredInput />}
                </Label>
                <div className="inline cursor-not-allowed">
                  <ToggleGroup type="single" value={streamType} disabled>
                    <ToggleGroupItem value={STREAM_TYPE.CAMERA}>
                      <Camera /> {t('live.StreamTypeWebcam')}
                    </ToggleGroupItem>
                    <ToggleGroupItem value={STREAM_TYPE.SOFTWARE}>
                      <Blocks /> {t('live.StreamTypeSoftware')}
                    </ToggleGroupItem>
                  </ToggleGroup>
                </div>
              </div>
          ...
          <DialogFooter className="sm:flex gap-1 w-full px-0 pt-3">
            ...
            {isCreateMode && (
              ...
              >
                <Radio />{' '}
                {isLoading ? t('live.StartingStream') : t('live.StartStream')}
              </Button>
            )}
            {isEditMode && (
              <Fragment>
                ...
                >
                  <Save /> {isLoading ? t('Saving') : t('Save')}
                </Button>
              </Fragment>
            )}
          </DialogFooter>
