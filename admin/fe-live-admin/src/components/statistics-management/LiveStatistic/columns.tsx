...
import { ColumnNames } from '@/components/statistics-management/LiveStatistic/columnData.ts';
...
export const getLiveStatisticTableColumns = ({
  sort,
}: ColumnsProps): ColumnDef<LiveStatisticsResponse>[] => [
  {
    accessorKey: 'title',
    header: () => (
      <DataTableColumnHeader
        title={<Trans i18nKey={ColumnNames.title.label} />}
        sort={{
  ...
  {
    accessorKey: 'status',
    header: () => <Trans i18nKey={ColumnNames.status.label} />,
    cell: ({ row }) => {
      const { status } = row.original;
      return <StatusBadge status={status} style="badge" />;
    },
  },
