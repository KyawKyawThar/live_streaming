...
import useVideoViewsStatistic, {
  TimePeriod,
} from '@/hooks/useVideoViewStatistic.ts';
import {
  Select,
  SelectContent,
  SelectItem,
...
const VideoViewChart = () => {
  ...
  const PERIOD_LABELS: Record<TimePeriod, string> = {
    ...
    '7days': t('dashboard.7Days'),
    '30days': t('dashboard.30Days'),
  };

  const selectItems = Object.entries(PERIOD_LABELS).map(([period, label]) => ({
    label,
        ...
        <div className="flex flex-1 items-center justify-between gap-1 px-6">
          <CardHeader className="px-0">
            <CardTitle className="text-xl text-left">
              {t('dashboard.VideoStatistics')}
            </CardTitle>
            <CardDescription className="text-left">
              {t('dashboard.VideoStatisticsDesc')}
            </CardDescription>
          </CardHeader>
          <Select
