...
const BlockAccount = ({
  ...
  return (
    <Dialog open={isOpen} onOpenChange={setOpen}>
      <DialogContent>
        <DialogHeader>
          <DialogTitle>{t('accountList.BlockAccount')}</DialogTitle>
        </DialogHeader>
        <div className="grid gap-1">
          <span className="font-medium">{t('accountList.User')}</span>
          ...
          <div className="gap-4 mt-3">
            <Label htmlFor="reason" className="text-right">
              {t('accountList.Reason')} <InputRequiredSymbol />
            </Label>
            <Textarea
              rows={3}
              placeholder={t('accountList.EnterReason')}
              id="reason"
              required
              value={reason}
