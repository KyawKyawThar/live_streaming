...
func BuildErrorResponse(ctx echo.Context, status int, err error, body interface{}, customizeError string) error {
	message := "An error occurred"
	if err != nil {
		message = err.Error()
	}
	return BuildStandardResponse(ctx, status, Response{Message: message, Error: customizeError, Code: status, Data: body})
}
