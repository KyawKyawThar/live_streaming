...
func (h *streamHandler) deleteLiveStream(c echo.Context) error {
	...
	if deletedStream.Stream.Status == model.STARTED {
		return utils.BuildErrorResponse(c, http.StatusBadRequest, fmt.Errorf("you can't delete stream while live"), nil, "INVALID_STREAM_STATUS")
	}
...
func (h *streamHandler) updateScheduledStreamByAdmin(c echo.Context) error {
	...
	if !utils.IsValidSchedule(req.ScheduledAt) {
		return utils.BuildErrorResponse(c, http.StatusBadRequest, errors.New("invalid schedule"), nil, "INVALID_SCHEDULE")
	}
	...
	if err != nil && strings.Compare(err.Error(), "http: no such file") != 0 {
		return utils.BuildErrorResponse(c, http.StatusBadRequest, err, fmt.Sprintf("upload video failed: %s", err.Error()), "INVALID_VIDEO")
	}
	...
	if video != nil {
		...
		if video.Size > utils.MAX_VIDEO_SIZE {
			return utils.BuildErrorResponse(c, http.StatusBadRequest, nil, "Video size exceeds the 2GB limit", "INVALID_SIZE")
		}

		isVideo, err := utils.IsVideoFile(video)
		if err != nil {
			return utils.BuildErrorResponse(c, http.StatusBadRequest, err, nil, "INVALID_VIDEO")
		}

		if !isVideo {
			return utils.BuildErrorResponse(c, http.StatusBadRequest, errors.New("file is not a supported video format"), nil, "INVALID_FORMAT")
		}
...
func (h *streamHandler) createLiveStreamByAdmin(c echo.Context) error {
	...
	if streamer == nil || streamer.Role.Type != model.STREAMER {
		return utils.BuildErrorResponse(c, http.StatusBadRequest, errors.New("user is not a streamer"), nil, "INVALID_USER")
	}
	...
	file, err := c.FormFile("thumbnail")
	if err != nil {
		return utils.BuildErrorResponse(c, http.StatusBadRequest, err, fmt.Sprintf("thumbnail field is required: %s", err.Error()), "INVALID_THUMBNAIL")
	}
	...
	video, err := c.FormFile("video")
	if err != nil {
		return utils.BuildErrorResponse(c, http.StatusBadRequest, err, fmt.Sprintf("video field is required: %s", err.Error()), "INVALID_VIDEO")
	}
...
func (h *streamHandler) endLiveStream(c echo.Context) error {
	...
	if stream.Status != model.STARTED {
		return utils.BuildErrorResponse(c, http.StatusBadRequest, errors.New("you can't end a live stream which is not started"), nil, "INVALID_STREAM_STATUS")
	}
