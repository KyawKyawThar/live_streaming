...
func (h *authHandler) login(c echo.Context) error {
	...
	if err != nil || user == nil || !utils.CheckPasswordHash(loginDTO.Password, user.PasswordHash) {
		return utils.BuildErrorResponse(c, http.StatusUnauthorized, errors.New("invalid username or password"), nil, "INVALID_USERNAME_OR_PASSWORD")
	}

	if user.Status == model.BLOCKED {
		return utils.BuildErrorResponse(c, http.StatusForbidden, errors.New("user was blocked"), nil, "BLOCKED_USER")
	}

	if !slices.Contains([]model.RoleType{model.ADMINROLE, model.SUPPERADMINROLE}, user.Role.Type) {
		return utils.BuildErrorResponse(c, http.StatusUnauthorized, errors.New("you have no permission to login"), nil, "PERMISSION_DENIED")
	}
	// update to online
...
func (h *authHandler) forgetPassword(c echo.Context) error {
	...
	if err != nil {
		return utils.BuildErrorResponse(c, http.StatusNotFound, errors.New("email not found"), nil, "NOT_FOUND")
...
func (h *authHandler) resetPassword(c echo.Context) error {
	...
	if resetPasswordDTO.NewPassword != resetPasswordDTO.ConfirmPassword {
		return utils.BuildErrorResponse(c, http.StatusBadRequest, errors.New("passwords do not match"), nil, "INVALID_PASSWORD")
	}
	...
	if time.Now().After(*user.OTPExpiresAt) {
		return utils.BuildErrorResponse(c, http.StatusBadRequest, errors.New("OTP expired"), nil, "OTP_EXPIRED")

	}
	if user.OTP != resetPasswordDTO.OTP {
		return utils.BuildErrorResponse(c, http.StatusBadRequest, errors.New("invalid OTP"), nil, "INVALID_OTP")
