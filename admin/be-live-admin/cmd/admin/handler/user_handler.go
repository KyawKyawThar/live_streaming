...
func (h *userHandler) byId(c echo.Context) error {
	...
	if err != nil {
		return utils.BuildErrorResponse(c, http.StatusBadRequest, errors.New("invalid id parameter"), nil, "INVALID_PARAMETER")
	}
	...
	if data == nil {
		return utils.BuildErrorResponse(c, http.StatusBadRequest, errors.New("not found"), nil, "NOT_FOUND")
	}
...
func (h *userHandler) deleteByID(c echo.Context) error {
	...
	if deletedUser == nil {
		return utils.BuildErrorResponse(c, http.StatusNotFound, errors.New("not found"), nil, "NOT_FOUND")
	}
...
func (h *userHandler) reactiveUser(c echo.Context) error {
	...
	if updatedUser.Role.Type == model.SUPPERADMINROLE {
		return utils.BuildErrorResponse(c, http.StatusBadRequest, errors.New("invalid request, super admin can't be reactive"), nil, "INVALID_ROLE")
	}

	if currentUser.RoleType == model.ADMINROLE && updatedUser.Role.Type == model.ADMINROLE {
		return utils.BuildErrorResponse(c, http.StatusBadRequest, errors.New("invalid request, admin can't reactive admin"), nil, "INVALID_ROLE")
	}
...
func (h *userHandler) deactiveUser(c echo.Context) error {
	...
	if err := utils.BindAndValidate(c, &request); err != nil {
		return utils.BuildErrorResponse(c, http.StatusBadRequest, err, nil, "INVALID_REQUEST")
	}
	...
	if updatedUser.Role.Type == model.SUPPERADMINROLE {
		return utils.BuildErrorResponse(c, http.StatusBadRequest, errors.New("invalid request, super admin can't be deactive"), nil, "INVALID_ROLE")
	}

	if currentUser.RoleType == model.ADMINROLE && updatedUser.Role.Type == model.ADMINROLE {
		return utils.BuildErrorResponse(c, http.StatusBadRequest, errors.New("invalid request, admin can't deactive admin"), nil, "INVALID_ROLE")
	}
...
func (h *userHandler) createUser(c echo.Context) error {
	...
	if err != nil && strings.Compare(err.Error(), "http: no such file") != 0 {
		return utils.BuildErrorResponse(c, http.StatusBadRequest, err, nil, "INVALID_IMAGE")
	}
	if file != nil {

		isImage, err := utils.IsImage(file)
		if err != nil {
			return utils.BuildErrorResponse(c, http.StatusBadRequest, err, nil, "INVALID_FORMAT")
		}

		if !isImage {
			return utils.BuildErrorResponse(c, http.StatusBadRequest, errors.New("file is not an image"), nil, "INVALID_FORMAT")
		}

		if file.Size > utils.MAX_IMAGE_SIZE {
			return utils.BuildErrorResponse(c, http.StatusBadRequest, nil, "Image size exceeds the 1MB limit", "INVALID_SIZE")
		}
	...
	if err := h.srv.User.CreateUser(&req); err != nil {
		return utils.BuildErrorResponse(c, http.StatusInternalServerError, err, nil, "INVALID_REQUEST")
	}
...
func (h *userHandler) changeAvatar(c echo.Context) error {
	...
	if file != nil {
		...
		if !isImage {
			return utils.BuildErrorResponse(c, http.StatusBadRequest, errors.New("file is not an image"), nil, "INVALID_IMAGE")
		}
		...
		dst, err := os.Create(avatarPath)
		if err != nil {
			return utils.BuildErrorResponse(c, http.StatusBadRequest, err, nil, "INVALID_PATH")
		}
		defer dst.Close()
	...
	if updatedUser == nil {
		return utils.BuildErrorResponse(c, http.StatusNotFound, err, nil, "INTERNAL_ERROR")
	}
...
func (h *userHandler) updateUser(c echo.Context) error {
	...
	if currentUser.RoleType == model.SUPPERADMINROLE {
		if currentUser.ID == uint(id) {
			return utils.BuildErrorResponse(c, http.StatusBadRequest, errors.New("invalid request, super admin don't update itself"), nil, "INVALID_ROLE")
		}
	} else {
		// validate admin user
		if currentUser.ID != uint(id) {
			return utils.BuildErrorResponse(c, http.StatusBadRequest, errors.New("invalid request, not allow update info of other account"), nil, "INVALID_ROLE")
		}
	}
...
func (h *userHandler) changePassword(c echo.Context) error {
	...
	if req.Password != req.ConfirmPassword {
		return utils.BuildErrorResponse(c, http.StatusBadRequest, errors.New("password not match confirm_password"), nil, "INVALID_PASSWORD")
	}
	...
	if currentUser.RoleType == model.ADMINROLE {
		if !slices.Contains([]model.RoleType{model.STREAMER, model.USERROLE}, targetUser.Role.Type) && currentUser.ID != uint(id) {
			return utils.BuildErrorResponse(c, http.StatusBadRequest, errors.New("invalid request, just allow change password self and streamer, user"), nil, "INVALID_ROLE")
		}
	}
